...
public final class StopTimeTravelSpeedValidatorTest {
  ...
  public void findDistancesKmBetweenStops_geoJsonLocation() {
    ...
    List<Optional<Map.Entry<String, GtfsTime>>> stopIdsAndTimes =
        ImmutableList.of(
            Optional.of(Maps.immutableEntry("s0", GtfsTime.fromString("14:48:00"))),
            // There's a GeoJSON location between the two stops.
            Optional.empty(),
            Optional.of(Maps.immutableEntry("s1", GtfsTime.fromString("14:56:00"))));
    ...
    for (int i = 0; i < stopIdsAndTimes.size(); ++i) {
      ...
      stopIdsAndTimes
          ...
          .ifPresent(
              t ->
                  stopTimeBuilder
                      .setArrivalTime(t.getValue())
                      .setDepartureTime(t.getValue())
                      .setStopId(t.getKey()));
